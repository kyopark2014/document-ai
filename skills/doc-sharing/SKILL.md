...
python3 /tmp/doc-sharing/scripts/share_artifact.py \
  --filepath "$ARTIFACTS_DIR/reports/<file>.xlsx" \
  --actor-id "<actor_id>"
```
...
- `filepath`는 실제 저장된 절대 경로를 넘긴다 (덮어쓰기·번호 접미사 반영된 path).
- `actor_id`는 시스템/분석 프롬프트 값을 그대로 사용한다 (닉네임·추측 금지).
- 다른 skill(regulation-evaluator 등)의 "업로드" 단계는 이 skill로 대체한다.

## 금지

- MCP `share_artifact` 호출을 시도하지 말 것 (이 프로젝트에는 없음)
- ESS CloudFront URL로 올리지 말 것 — **document-ai** `S3_BUCKET` / `SHARING_URL`만 사용
- S3 콘솔 URL·프리사인만 주고 CloudFront URL을 빼먹지 말 것 (`SHARING_URL`이 있으면 반드시 CF URL)
